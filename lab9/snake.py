...
class Food:
    def __init__(self):
        self.position = (0, 0)
        self.timer = 100  # Set the initial timer value
        self.weights = [30, 40]  # Possible weights for the food
        self.weight = 0  # Actual weight of the food
        self.spawn()
    ...
    def spawn(self):
        while True:
            ...
            if (x, y) not in snake.body and (x, y) not in walls:
                self.position = (x, y)
                self.timer = 20  # Reset the timer each time the food spawns
                self.weight = random.choice(self.weights)  # Randomly assign a weight
                break
    ...
    def update(self):
        if self.timer > 0:
            ...
            self.spawn()  # Respawn the food when the timer reaches zero
...
while running:
    ...
    def draw_food():
        x, y = food.position
        size = food.weight  # Используем вес еды для определения размера
